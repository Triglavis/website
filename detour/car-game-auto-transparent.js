(() => {
  ...
  function updateTransmission(dt) {
    ...
    if (car.isShifting) {
      ...
      if (car.shiftProgress >= 1.0) {
        ...
        // Add lurch effect on gear shift
        const lurchForce = 0.2;
        car.vx *= (1 - lurchForce);
        car.vy *= (1 - lurchForce);
      } else {
  ...
  function init() {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      // Removed manual shifting - automatic only
  ...
  function drawUI() {
    ...
    // Show state instead of mode
    ctx.fillText(`STATE: ${gameState.toUpperCase()}`, size * 25, size * 1.5);
