(() => {
  ...
  let gameState = {
    // Position
    x: 30,      // Start off-screen to the right for intro
    z: 0,       // Center of screen
    y: 0.5,     // Ground level
    ...
    maxTorque: 400,  // Nm - High torque for good acceleration
  ...
  function createTreasureChest() {
    ...
    // Position chest ahead of the car's spawn position
    // Place treasure in the fenced area
    chestGroup.position.set(0, 0, -100); // In fenced area (half the original distance)
  ...
  function checkFenceCollisions() {
    ...
    fence.walls.forEach(wall => {
      ...
      if (wallBox.intersectsBox(carBox)) {
        // Calculate wall world position
        const wallWorldPos = wall.position.clone();
        wallWorldPos.add(fence.position);
        ...
        if (isNorthSouth) {
          ...
          if (distanceToWall < minSeparation) {
            ...
            // Push just enough to separate, plus a small buffer
            gameState.z += pushDirection * (overlap + 0.1);
          }
          
          // Reverse and dampen Z velocity
          gameState.vz = -gameState.vz * 0.3;
          
          // Add some lateral bounce based on approach angle
          const lateralComponent = gameState.vx * 0.1;
          gameState.vx += lateralComponent;
        } else {
          ...
          if (distanceToWall < minSeparation) {
            const overlap = minSeparation - distanceToWall;
            // Push just enough to separate, plus a small buffer
            gameState.x += pushDirection * (overlap + 0.1);
          }
          
          // Reverse and dampen X velocity
          gameState.vx = -gameState.vx * 0.3;
          
          // Add some lateral bounce based on approach angle
          const lateralComponent = gameState.vz * 0.1;
          gameState.vz += lateralComponent;
        }
        
        // Reduce wheel speed but not as much
        gameState.wheelSpeed *= 0.7;
        gameState.frontWheelSpeed *= 0.7;
  ...
  function createTreasureTooltip() {
    ...
    treasureTooltip.position.set(0, 1.5, -100);  // Above treasure chest in fence (adjusted for new position)
  ...
  function createControlPanel() {
    ...
    const panelHTML = `
      ...
      <div class="dev-content">
        ...
        <div class="dev-section active" id="section-visualization">
          <div class="dev-section-header" onclick="toggleDevSection('visualization')">
        ...
        <div class="dev-section" id="section-position">
          <div class="dev-section-header" onclick="toggleDevSection('position')">
            <span>📍 Position & Movement</span>
            <span class="section-arrow">▶</span>
          </div>
          <div class="dev-section-content">
        ...
        <!-- Camera Section -->
        <div class="dev-section" id="section-camera">
          <div class="dev-section-header" onclick="toggleDevSection('camera')">
            <span>📷 Camera</span>
            <span class="section-arrow">▶</span>
          </div>
          <div class="dev-section-content">
        ...
        <!-- Physics Section -->
        <div class="dev-section" id="section-physics">
          <div class="dev-section-header" onclick="toggleDevSection('physics')">
            <span>⚙️ Physics</span>
            <span class="section-arrow">▶</span>
          </div>
          <div class="dev-section-content">
        ...
        <!-- Debug Section -->
        <div class="dev-section" id="section-debug">
          <div class="dev-section-header" onclick="toggleDevSection('debug')">
            <span>🐛 Debug</span>
            <span class="section-arrow">▶</span>
          </div>
          <div class="dev-section-content">
            ...
            <div class="dev-control" style="margin-top: 12px;">
              <label class="dev-checkbox-label">
  ...
  // Camera zoom level
  let cameraZoom = 1.2;  // Default zoom level 1.2x
  ...
  function init() {
    ...
    scene.background = new THREE.Color(CONFIG.colors.white); // White background
    ...
    function update(deltaTime) {
      ...
      // For now, just close the update function properly
    }
  ...
  function update(deltaTime) {
    ...
    if (hasGroundContact) {
      ...
      if (Math.abs(gameState.wheelAngle) > 0.001 && Math.abs(newForwardSpeed) > 0.01) {
        // Speed-sensitive steering reduction (more reduction at higher speeds)
        const speedKmh = Math.abs(newForwardSpeed) * 3.6; // Convert m/s to km/h
        const steerReduction = 1 / (1 + speedKmh * 0.04); // At 50km/h, steering is reduced by ~67% (doubled from 0.02)
        const effectiveSteerAngle = gameState.wheelAngle * steerReduction;
        
        // Calculate ideal turning radius using Ackermann steering geometry
        const minRadius = 5.0; // Minimum turning radius in meters
        const idealRadius = Math.max(minRadius, gameState.wheelbase / Math.tan(Math.abs(effectiveSteerAngle)));
        
        // Calculate centripetal force required for the turn
        const centripetalAccel = (newForwardSpeed * newForwardSpeed) / idealRadius;
        const lateralForceRequired = gameState.mass * centripetalAccel;
        
        // Calculate available tire grip (sum of all tire lateral forces)
        const totalWeight = gameState.mass * gameState.gravity;
        const maxLateralForce = totalWeight * 0.8; // Approximate max lateral grip
        
        // Scale turning based on available grip vs required force
        const gripFactor = Math.min(1.0, maxLateralForce / lateralForceRequired);
        
        // Apply understeer when grip is exceeded - turn radius increases
        const actualRadius = idealRadius / gripFactor;
        
        // Account for rotational inertia - larger radius at higher speeds due to momentum
        const inertiaFactor = 1.0 + (speedKmh / 100) * 0.5; // Up to 50% larger turn radius at 100km/h
        const finalRadius = actualRadius * inertiaFactor;
        
        // Angular velocity = linear velocity / turning radius
        // Note: Negative because rotation.y = -angle in rendering
        const targetAngularVelocity = -newForwardSpeed / finalRadius * Math.sign(effectiveSteerAngle);
        
        // Apply rotational inertia - slower response at higher speeds
        const rotationalInertia = gameState.momentOfInertia * (1 + speedKmh * 0.01);
        const maxAngularAccel = maxLateralForce * gameState.wheelbase / rotationalInertia;
        
        // Calculate angular acceleration limited by physics
        const desiredAngularAccel = (targetAngularVelocity - gameState.angularVelocity) * 3.0;
        const angularAccel = Math.max(-maxAngularAccel, Math.min(maxAngularAccel, desiredAngularAccel));
        
        gameState.angularVelocity += angularAccel * deltaTime;
      } else {
    ...
    if (carGroup.wheels) {
      ...
      const suspensionPitch = (rearCompression - frontCompression) * 1.0; // Reduced from 2.5
  ...
  function expandBrowserCountry() {
    ...
    function animateExpansion() {
      ...
      // Move treasure further away
      const treasure = scene.getObjectByName('treasure');
      if (treasure) {
        treasure.position.z = fence.position.z - 30 * fence.scale.z; // Deep inside expanded area
      }
